...
| latest | `fetchLatestUpdates(1)` | success | 35 | 노아 선배는 친구 (`.../board.php <redacted query values: bo_table, stx, and is>`) |  | 1-10s |
| latest_next | `fetchLatestUpdates(2)` | success | 35 | 완벽한 마녀는 오늘도 허당!? (`.../board.php <redacted query values: bo_table, stx, and is>`) |  | <1s |
...
| chapters | `fetchChapterList(manga)` | success | 314 | 열혈강호 1권 (`.../board.php <redacted query values: bo_table, wr_id, stx, and is>`) |  | 1-10s |
...
| duplicate thumbnail URLs | UNUSUAL | Thumbnail URLs used by different manga URLs: `https://11toon8.com/.../1.webp` -> 2 manga URLs (examples: `.../board.php <redacted query values: bo_table, stx, and is>`, `.../board.php <redacted query values: bo_table, stx, and is>`), `https://11toon8.com/.../33394.webp` -> 2 manga URLs (examples: `.../board.php <redacted query values: bo_table and stx> 구멍은 있다!&is=33394`, `.../board.php <redacted query values: bo_table, stx, and is>`) |  |  |  |
